...
import ai.skymind.pipelines.pipeline.handlers.converter.multi.converter.impl.arrow.ArrowBinaryInputAdapter;
import ai.skymind.pipelines.pipeline.handlers.converter.multi.converter.impl.image.VertxBufferImageInputAdapter;
import ai.skymind.pipelines.pipeline.handlers.converter.multi.converter.impl.nd4j.VertxBufferNd4jInputAdapter;
import ai.skymind.pipelines.pipeline.handlers.converter.multi.converter.impl.numpy.VertxBufferNumpyInputAdapter;
...
import static ai.skymind.pipelines.verticles.VerticleConstants.CONVERTED_INFERENCE_DATA;
...
public class InferenceVerticle extends BaseRoutableVerticle {
    ...
    public void init(Vertx vertx, Context context) {
        ...
        if(inferenceConfiguration.getServingConfig().getMetricTypes() != null && registry != null) {
            ...
            for(MetricType metricType : new HashSet<>(inferenceConfiguration.getServingConfig().getMetricTypes())) {
                switch(metricType) {
                    ...
                    case GPU:
                        try {
                            MeterBinder meterBinder = (MeterBinder) Class.forName("ai.skymind.pipelines.gpu.GpuMetrics").newInstance();
        ...
        router.post("/:operation/:inputType")
            ...
            ctx.vertx().executeBlocking(handler -> {
                ...
                ctx.put(CONVERTED_INFERENCE_DATA,batch);
                handler.complete();
        ...
        router.post("/:operation/:inputType")
            ...
            Record[] inputs = ctx.get(CONVERTED_INFERENCE_DATA);
    ...
    protected void setupWebServer() {
        Integer portValue = inferenceConfiguration.getServingConfig().getHttpPort();

        vertx.createHttpServer()
                .requestHandler(router)
                .exceptionHandler(Throwable::printStackTrace)
                .listen(portValue, inferenceConfiguration.getServingConfig().getListenHost(), listenResult -> {
                    if (listenResult.failed()) {
                        log.debug("Could not start HTTP server");
                        listenResult.cause().printStackTrace();
                    } else {
                        log.debug("Server started on port " + portValue);
                    }
                });
    }
