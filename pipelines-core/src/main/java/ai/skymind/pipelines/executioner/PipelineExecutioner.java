...
import ai.skymind.pipelines.model.TensorDataType;
import ai.skymind.pipelines.model.TensorDataTypesConfig;
import ai.skymind.pipelines.output.adapter.*;
import ai.skymind.pipelines.output.types.BatchOutput;
import ai.skymind.pipelines.output.types.NDArrayOutput;
import ai.skymind.pipelines.pipeline.*;
import ai.skymind.pipelines.pipeline.handlers.converter.JsonArrayMapConverter;
import ai.skymind.pipelines.pipeline.steps.InferenceExecutionerPipelineStepRunner;
import ai.skymind.pipelines.serving.Output;
import ai.skymind.pipelines.serving.ServingConfig;
import ai.skymind.pipelines.util.ArrowUtils;
...
public class PipelineExecutioner {
     ...
     */
    public void doInference(io.vertx.ext.web.RoutingContext ctx, ai.skymind.pipelines.serving.Output.DataType responseOutputType, org.datavec.api.records.Record[] inputs) {
    ...
    private void writeBinary(Buffer buffer, String batchId, RoutingContext ctx) {
        try {
            ctx.response().putHeader(VerticleConstants.BATCH_ID_HEADER, batchId);
            ctx.response().putHeader("Content-Type", "application/octet-stream");
