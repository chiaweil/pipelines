...
import ai.skymind.pipelines.executioner.inference.factory.InferenceExecutionerFactory;
    ...
    public InferenceExecutionerPipelineStepRunner(PipelineStep pipelineStep) {
        ...
        try {
            ...
            switch (modelPipelineStepConfig.getModelConfig().getModelConfigType().getModelType()) {
                case SAMEDIFF:
                    inferenceExecutionerFactory = (InferenceExecutionerFactory) Class.forName("ai.skymind.pipelines.executioner.inference.factory.SameDiffInferenceExecutionerFactory").newInstance();
                    break;
                case TENSORFLOW:
                    inferenceExecutionerFactory = (InferenceExecutionerFactory) Class.forName("ai.skymind.pipelines.executioner.inference.factory.TensorflowInferenceExecutionerFactory").newInstance();
                    break;
                case COMPUTATION_GRAPH:
                    inferenceExecutionerFactory = (InferenceExecutionerFactory) Class.forName("ai.skymind.pipelines.executioner.inference.factory.ComputationGraphInferenceExecutionerFactory").newInstance();
                    break;
                case MULTI_LAYER_NETWORK:
                    inferenceExecutionerFactory = (InferenceExecutionerFactory) Class.forName("ai.skymind.pipelines.executioner.inference.factory.MultiLayerNetworkInferenceExecutionerFactory").newInstance();
                    break;
                case PMML:
                    inferenceExecutionerFactory = (InferenceExecutionerFactory) Class.forName("ai.skymind.pipelines.executioner.inference.factory.PmmlInferenceExecutionerFactory").newInstance();
                    break;
                case KERAS:
                    inferenceExecutionerFactory = (InferenceExecutionerFactory) Class.forName("ai.skymind.pipelines.executioner.inference.factory.KerasInferenceExecutionerFactory").newInstance();
                    break;
                default:
